...
export default function EditTransactionModal({ year, month, day, dayId, transactions = [], onClose }) {
  ...
  useEffect(() => {
    setRows(
      transactions.length
        ...
        : [{ id: generateRowId(), category: "", amount: "", comments: "", isNew: true }]
    );
  }, [transactions]);
  ...
  const addRow = () => {
    const newRow = { id: generateRowId(), category: "", amount: "", comments: "", isNew: true };
  ...
  const removeRow = (id) => {
    ...
    // mark the row as deleted tentatively
    if (!row.isNew) setDeletedRows((prev) => [...prev, id]);

    // remove from the displayed rows
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="bg-gray-100">
                ...
                <th className="border px-3 py-2 w-[55%]">Amount (comma-separated)</th>
